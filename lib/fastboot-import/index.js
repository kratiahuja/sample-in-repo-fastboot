...
module.exports = {
  ...
  treeForVendor(tree) {
    ...
    const primusDir = path.join(this.project.root, 'primus');
    console.log(primusDir);
    if (existSync(primusDir)) {
      // Funnel the browser lib from bower with providing destDir as the lib (this is optional). If you don't
      // provide `destDir` it will default to `vendor/yourlib.js`. If you provide destDir it will default to:
      // `vendor/destDirName/yourlib.js`
      const browserTrees = fastbootTransform(new Funnel(primusDir, {
         files: ['foo.js'],
         destDir: 'primus'
       }));
       trees.push(browserTrees);
  ...
  included(app) {
    ...
    app.import('vendor/primus/foo.js');
  }
};
